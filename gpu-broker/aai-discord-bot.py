...
async def generate_image_collage(prompt):
    # Use portrait dimensions, add quality boosters
    full_prompt = prompt + QUALITY_SUFFIX
    async with aiohttp.ClientSession() as session:
        tasks = [asyncio.ensure_future(fetch_image(session, full_prompt)) for _ in range(4)]
...
class RetryButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        try:
            image_bytes_list = await generate_image_collage(self.prompt)
            if image_bytes_list:
...
class AaiCog(commands.Cog, name="aai"):
    ...
    async def generate_with_progress(self, prompt, progress_msg):
        """Generate 4 images with live progress updates."""
        full_prompt = prompt + QUALITY_SUFFIX
        completed = []
        total = 4

        async def gen_one(session, idx):
            result = await fetch_image(session, full_prompt)
            if result:
                result = apply_watermark(result)
                completed.append(result)
                done = len(completed)
                bars = "🟩" * done + "⬜" * (total - done)
                try:
                    await progress_msg.edit(
                        content=f"🎨 **Generating 4x images** for: `{prompt}`\n"
                               f"{bars} {done}/{total} complete..."
                    )
                except Exception:
                    pass
            return result
    ...
    async def process_queue(self):
        while True:
            interaction, prompt = await self.request_queue.get()
            try:
                # Show progress message with placeholder
                progress_msg = await interaction.followup.send(
                    f"🎨 **Generating 4x images** for: `{prompt}`\n"
                    f"⬜⬜⬜⬜ Starting generation..."
                )

                # Generate with progress tracking
                image_bytes_list = await self.generate_with_progress(prompt, progress_msg)

                if not image_bytes_list or len(image_bytes_list) == 0:
                    await progress_msg.edit(
                        content=f"⚠️ First attempt returned no images for: `{prompt}` — retrying..."
                    )
                    image_bytes_list = await self.generate_with_progress(prompt, progress_msg)

                if image_bytes_list and len(image_bytes_list) == 4:
                    collage_buffer = create_collage(image_bytes_list)
                    if collage_buffer:
                        # Update progress message to done
                        try:
                            await progress_msg.edit(content=f"🟩🟩🟩🟩 **4/4 complete!** Building collage...")
                        except Exception:
                            pass
                        file = discord.File(collage_buffer, filename="collage.png")
                        view = ImageButtons(image_bytes_list, prompt)
                        message = await interaction.followup.send(
                            f"✅ **4x images** for: `{prompt}`\nClick 1-4 to expand · 🔄 Retry · 🗑 Delete",
                            file=file, view=view
                        )
                        ...
                        await interaction.followup.send("Failed to create collage.")
                elif image_bytes_list and len(image_bytes_list) >= 1:
                    got = len(image_bytes_list)
                    files = [discord.File(io.BytesIO(img), filename=f"image_{i+1}.png") for i, img in enumerate(image_bytes_list)]
                    view = ImageButtons(image_bytes_list, prompt)
                    message = await interaction.followup.send(
                        f"⚠️ **{got}/4 images** for: `{prompt}` (GPU was busy)\nClick to expand · 🔄 Retry",
                    ...
                    try:
                        await progress_msg.edit(content="❌ Could not generate images. The GPU may be overloaded — try again in a moment.")
                    except Exception:
                        await interaction.followup.send("Could not generate images. The GPU may be overloaded — try again in a moment.")
    ...
    @app_commands.command(name="aai", description="Generate 4 AI images from a prompt.")
    @app_commands.describe(prompt="The image prompt to generate.")
    async def aai(self, interaction: discord.Interaction, prompt: str):
        await interaction.response.defer()
        await self.request_queue.put((interaction, prompt))
        pos = self.request_queue.qsize()
        await interaction.followup.send(f"🎨 Queued at position {pos} — generating **4x images** shortly...")
