...
# Available LoRAs (synced with GPU server registry)
AVAILABLE_LORAS = {
    "more_details": "Add More Detail (enhances textures)",
    "detail_tweaker": "Detail Tweaker (fine detail control)",
    "epi_noiseoffset": "Dramatic Lighting (deeper shadows)",
    "clothing_adjuster": "Clothing Adjuster (+/- clothing)",
    "polaroid_style": "Polaroid / Vintage Photo",
    "ghibli_style": "Ghibli / Anime Art",
    "cute_girl_mix4": "Soft / Cute Aesthetic",
    "anime_lineart": "Anime Lineart / Manga",
    "hipoly_3d": "3D Rendered / CGI Look",
}
...
async def fetch_image(session, prompt, gen_opts=None):
    ...
    # LoRA support
    if "lora_id" in opts and opts["lora_id"]:
        data["lora_id"] = opts["lora_id"]
        if "lora_strength" in opts:
            data["lora_strength"] = opts["lora_strength"]
    if "loras" in opts and opts["loras"]:
        data["loras"] = opts["loras"]
    try:
        async with session.post(
            IMAGE_API_URL,
            json=data,
...
class AnimateButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        await interaction.response.defer(thinking=True)
        await interaction.followup.send("🎬 Animating... (~5s) — powered by **AdultAI.com**", ephemeral=True)
        try:
            import uuid as _uuid
            async with aiohttp.ClientSession() as session:
                # Upload source image to Bunny CDN first
                uid = str(_uuid.uuid4())[:8]
                style = random.choice(["zoom_in", "zoom_out", "pan_right", "pan_up", "drift"])
                image_url = await upload_to_bunny(session, self.image_bytes, f"img/anim_src_{uid}.png", "image/png")

                # Call animate API
                async with session.post(
                    f"{GPU_API_URL}/api/v1/animate",
                    json={"image_url": image_url, "style": style, "duration": 3.0, "fps": 12},
                    headers={"X-API-Key": GPU_API_KEY, "Content-Type": "application/json"},
                    timeout=aiohttp.ClientTimeout(total=60),
                ) as r:
                    if not r.ok:
                        await interaction.followup.send("❌ Animation failed — try again.", ephemeral=True)
                        return
                    data = await r.json()
                    result_url = data.get("video_url")
                    if not result_url:
                        await interaction.followup.send("❌ No video returned.", ephemeral=True)
                        return

                # Prefer GIF (auto-plays inline in Discord), fallback to MP4
                gif_url = data.get("gif_url")
                if gif_url:
                    async with session.get(gif_url) as gr:
                        gif_bytes = await gr.read()
                    file = discord.File(io.BytesIO(gif_bytes), filename="animation.gif")
                else:
                    async with session.get(result_url) as vr:
                        video_bytes = await vr.read()
                    file = discord.File(io.BytesIO(video_bytes), filename="animation.mp4")

            await interaction.followup.send(
                f"🎬 **Animated!** *(style: {style})* More at **AdultAI.com** ✨",
                file=file,
            )
        except Exception as e:
            print(f"[ANIMATE BUTTON ERROR] {e}")
            await interaction.followup.send("❌ Animation failed.", ephemeral=True)
...
class FaceFixButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            save_view = SingleSaveView(fixed_bytes, result_url, prompt="Face Fixed")
            msg = await interaction.followup.send("💆 **Face fixed!** More tools at **AdultAI.com**", file=file, view=save_view)
            save_view.message = msg
        except Exception as e:
...
class UpscaleButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        await interaction.response.defer(thinking=True)
        await interaction.followup.send(f"\u2b06\ufe0f Upscaling {self.scale}x... (~20s)", ephemeral=True)
        try:
            import uuid as _uuid
            async with aiohttp.ClientSession() as session:
                # Upload source image to Bunny CDN first (GPU API requires a URL)
                uid = str(_uuid.uuid4())[:8]
                image_url = await upload_to_bunny(session, self.image_bytes, f"img/upscale_src_{uid}.png", "image/png")

                # Call GPU upscale API with URL
                async with session.post(
                    f"{GPU_API_URL}/api/v1/upscale",
                    json={"image_url": image_url, "scale": self.scale, "enhance": True},
                    headers={"X-API-Key": GPU_API_KEY, "Content-Type": "application/json"},
                    timeout=aiohttp.ClientTimeout(total=120),
                ) as r:
                    if r.ok:
                        ...
                        result_url = data.get("image_url")
                        if result_url:
                            # Download result and send as Discord file
                            async with session.get(result_url) as img_r:
                                result_bytes = await img_r.read()
                            file = discord.File(io.BytesIO(result_bytes), filename=f"upscaled_{self.scale}x.png")
                            save_view = SingleSaveView(result_bytes, result_url, prompt=f"Upscaled {self.scale}x")
...
class SingleSaveView(discord.ui.View):
    """Minimal view with just a Save button - used on upscale/edit/facefix results."""
    def __init__(self, image_bytes: bytes, image_url: str, prompt: str = ""):
        super().__init__(timeout=None)
        self.message = None
        self.add_item(SaveToAdultAIButton(image_bytes, image_url, prompt=prompt))



# ── Category taxonomy (matches AdultAI.com canonical set) ────────────────────
CATEGORY_SYNONYMS = {
    # === ACTS ===
    "Blowjob":       ["blowjob","blow job","oral sex","sucking cock","deepthroat","fellatio","face fuck"],
    "Anal":          ["anal","anal sex","ass fuck","butt fuck","anal penetration","pegging"],
    "Cumshot":       ["cumshot","cum shot","cum on face","facial","cum on tits","creampie","covered in cum"],
    "Masturbation":  ["masturbat","fingering","touching herself","dildo","vibrator","sex toy","rubbing herself"],
    "Lesbian":       ["lesbian","girl on girl","two girls","two women","sapphic","eating out","scissoring","girls kissing","strap on"],
    "Hardcore":      ["gangbang","group sex","double penetration","orgy","cock","fucking","fuck ","penetrat","intercourse","riding cock","titfuck","spread legs"],
    "Couples":       ["couple","making love","in an embrace","romantic partners","sex with","fucking","missionary","doggy","cowgirl","mating press"],
    "BDSM":          ["bondage","tied up","bound","restrained","rope","collar","dominant","submissive","mistress","whip","spanking","chains","gag","blindfold","dungeon","slave"],
    "Femdom":        ["femdom","female dominant","dominant woman","dominatrix","face sitting","chastity","pegging"],
    "Futanari":      ["futa","futanari","dickgirl","shemale","ladyboy"],
    "Masturbation":  ["masturbat","fingering herself","touching herself","dildo","vibrator","playing with herself"],
    "POV":           ["pov","point of view","first person","eye contact with camera"],
    "Erotic":        ["erotic","sensual","seductive","intimate","tease","alluring","provocative","sultry"],

    # === FANTASY / COSTUME ===
    "Anime":         ["anime","hentai","manga","kawaii","waifu","ecchi","ahegao","2d art","cel shaded","tentacle"],
    "Cartoon":       ["cartoon","animated character","smurf","pixar style","disney style","comic character"],
    "Vampire":       ["vampire","fangs","undead gothic","dracula"],
    "Elf":           ["elf ","elven","pointed ears","dark elf","wood elf"],
    "Demon":         ["demon","succubus","devil girl","horns and wings","demonic"],
    "Angel":         ["angel","white wings","celestial","halo","angelic"],
    "Witch":         ["witch","sorceress","witch hat","cauldron","mage","spellcaster"],
    "Furry":         ["furry","anthro","fox girl","wolf girl","cat girl anthro","neko girl"],
    "Fantasy":       ["fantasy world","magical realm","enchanted","mystical","ethereal being","dark fantasy","dragon","medieval"],
    "Cosplay":       ["cosplay","dressed as","roleplay costume","character costume","anime cosplay","superhero"],
    "Maid":          ["maid","french maid","maid outfit","maid uniform"],
    "Nurse":         ["nurse","nurse outfit","nurse costume","medical uniform"],
    "Schoolgirl":    ["schoolgirl","school uniform","plaid skirt","sailor uniform","student uniform"],
    "Bunny":         ["bunny girl","bunny ears","playboy bunny","rabbit costume"],
    "Sofonda Cox":   ["sofondacox","sofonda cox","sofanda"],

    # === BODY TYPE ===
    "Busty":         ["busty","big tits","large breasts","huge boobs","massive tits","buxom","ample bosom"],
    "Curvy":         ["curvy","voluptuous","thick woman","big ass","big booty","hourglass","plus size","bbw","chubby","plump"],
    "BBW":           ["bbw","plus size","chubby woman","full figured","plump woman"],
    "Petite":        ["petite","tiny girl","small frame","slender woman","slim woman","small breasts"],
    "Athletic":      ["athletic","toned","muscular woman","abs","fit body","gym body","yoga body","sports bra","workout","athlete"],
    "Muscular":      ["muscular","jacked","buff","ripped","bodybuilder","biceps","strong woman"],
    "Mature":        ["mature woman","older woman","cougar","experienced woman","mom","mommy","stepmom"],
    "Milf":          ["milf","milf ","hot mom","sexy mom","hot wife"],

    # === ETHNICITY / HAIR ===
    "Asian":         ["asian","japanese","chinese","korean","thai","filipina","kimono","qipao"],
    "Latina":        ["latina","hispanic","mexican","colombian","brazilian","latin woman"],
    "Ebony":         ["ebony","black woman","dark skin","melanin","african american","nubian"],
    "Redhead":       ["redhead","red hair","ginger","auburn hair","fiery hair"],
    "Blonde":        ["blonde","blond","platinum blonde","golden hair"],
    "Brunette":      ["brunette","brown hair","dark hair","black hair","raven hair"],

    # === OUTFIT ===
    "Lingerie":      ["lingerie","bra and panties","lace corset","thong","garter","corset","babydoll","negligee","lacy","underwear"],
    "Bikini":        ["bikini","swimsuit","two piece","swimwear","bathing suit","micro bikini"],
    "Stockings":     ["stockings","fishnet stockings","thigh highs","nylons","hold ups"],
    "Latex":         ["latex","rubber suit","pvc catsuit","shiny suit","latex dress","leather suit"],
    "Naked":         ["nude","naked","fully nude","completely naked","topless","bare body","undressed","no clothes"],

    # === ART STYLE ===
    "Glamour":       ["glamour","editorial","high fashion","runway","vogue style","magazine shoot"],
    "Fine Art":      ["fine art","figure study","artistic photograph","boudoir photography"],
    "Artistic Nude": ["artistic nude","nude art","fine art nude","body art","flowers over her body"],
    "Vintage":       ["vintage","retro","polaroid","film grain","old photograph","sepia"],
    "Pin-Up":        ["pin-up","pinup","cheesecake","classic pin up","1950s glamour"],

    # === SETTING ===
    "Bedroom":       ["bedroom","on the bed","satin sheets","in bed","boudoir","duvet"],
    "Beach":         ["beach","ocean","shore","sunbathing","tropical beach","by the sea"],
    "Poolside":      ["poolside","by the pool","pool party","pool deck","hot tub","jacuzzi"],
    "Outdoor":       ["outdoor","outside","nature","forest","park","garden","open air","field"],
    "Office":        ["office","at her desk","secretary","corporate","boardroom"],
    "Kitchen":       ["kitchen","counter","apron","cooking","chef"],
    "Shower":        ["shower","steamy shower","under the shower","bathtub","soaking","wet and soapy"],
    "Bar":           ["bar ","pub ","bartender","nightclub","club lounge","rave","dance floor","nightlife"],
    "Rooftop":       ["rooftop","city view","skyline","roof terrace","penthouse terrace","penthouse"],
    "Cyberpunk":     ["cyberpunk","neon lights","neon city","futuristic","sci-fi","android","cyborg","neon glow"],
}


def suggest_categories(prompt: str, max_cats: int = 5) -> list[str]:
    """Return up to max_cats best-matching niche category names from a prompt."""
    p = prompt.lower()
    scores: dict[str, int] = {}
    EXCLUDE = {"smurf", "Public"}
    for cat, syns in CATEGORY_SYNONYMS.items():
        if cat in EXCLUDE:
            continue
        for syn in syns:
            if syn in p:
                pts = 2 if " " in syn else 1
                scores[cat] = scores.get(cat, 0) + pts
    ranked = sorted(scores.items(), key=lambda x: x[1], reverse=True)
    return [name for name, score in ranked[:max_cats] if score > 0]

def suggest_category(prompt: str) -> str:
    """Return the single best category name (legacy compat)."""
    cats = suggest_categories(prompt, 1)
    return cats[0] if cats else ""


class SaveModal(discord.ui.Modal, title="Save to AdultAI.com"):
    """Save modal with auto-detected category pre-filled. Category is required."""

    # All valid category names for hint display
    ALL_CATEGORIES = [
        # Acts
        "Anal","BDSM","Blowjob","Couples","Cumshot","Erotic","Femdom","Futanari",
        "Hardcore","Lesbian","Masturbation","POV",
        # Fantasy / Costume
        "Angel","Anime","Bunny","Cartoon","Cosplay","Demon","Elf","Fantasy",
        "Furry","Maid","Nurse","Schoolgirl","Sofonda Cox","Vampire","Witch",
        # Body Type
        "Athletic","BBW","Busty","Curvy","Mature","Milf","Muscular","Petite",
        # Ethnicity / Hair
        "Asian","Blonde","Brunette","Ebony","Latina","Redhead",
        # Outfit
        "Bikini","Latex","Lingerie","Naked","Stockings",
        # Art Style
        "Artistic Nude","Fine Art","Glamour","Pin-Up","Vintage",
        # Setting
        "Bar","Beach","Bedroom","Cyberpunk","Kitchen","Office","Outdoor",
        "Poolside","Rooftop","Shower",
    ]
    ...
    category_input = discord.ui.TextInput(
        label="Category  (edit or keep suggested)",
        placeholder="e.g. Erotic",
        min_length=1,
        max_length=40,
        required=True,
    )
    hint_input = discord.ui.TextInput(
        label="Valid categories (reference, do not edit)",
        style=discord.TextStyle.paragraph,
        placeholder="",
        required=False,
        max_length=4000,
    ...
    def __init__(self, image_bytes: bytes, image_url: str, prompt: str):
        ...
        self.image_url = image_url
        self.prompt = prompt
        # Auto-detect top categories from prompt
        top_cats = suggest_categories(prompt, 3)
        best = top_cats[0] if top_cats else "Erotic"
        # Pre-fill with best match; show top suggestions in label
        self.category_input.default = best
        if len(top_cats) > 1:
            label = f"Category (suggested: {' · '.join(top_cats[:3])})"
            self.category_input.label = label[:45]  # Discord label max 45 chars
        # Populate hint field with sorted canonical list
        hint_text = "  ·  ".join(self.ALL_CATEGORIES)
        self.hint_input.default = hint_text

    async def on_submit(self, interaction: discord.Interaction):
        await interaction.response.defer(thinking=True, ephemeral=True)

        # Visibility
        vis_raw = self.visibility_input.value.strip().lower()
        is_public = vis_raw != "private" and vis_raw != "2"

        # Category — strip # prefix, title-case, fall back to auto-detect
        raw_cat = self.category_input.value.strip().lstrip("#").strip()
        # Try exact match (case-insensitive) against known categories
        category_name = next(
            (c for c in self.ALL_CATEGORIES if c.lower() == raw_cat.lower()),
            None
        )
        if not category_name:
            # Partial match — find first category that starts with what they typed
            category_name = next(
                (c for c in self.ALL_CATEGORIES if c.lower().startswith(raw_cat.lower())),
                None
            )
        if not category_name:
            # Still nothing — fall back to auto-detect from prompt
            category_name = suggest_category(self.prompt) or "Erotic"
        ...
        async with aiohttp.ClientSession() as session:
            if not self.image_url or not self.image_url.startswith("http"):
                uid = f"{int(time.time())}_{random.randint(1000, 9999)}"
                self.image_url = await upload_to_bunny(session, self.image_bytes, f"img/discord_save_{uid}.png", "image/png")

            payload = {
                "discordId": str(discord_user.id),
                "discordUsername": discord_user.display_name or discord_user.name,
                "imageUrl": self.image_url,
                "prompt": self.prompt or "Discord generation",
                "isPublic": is_public,
                "categoryName": category_name,
            }
            req_headers = {
                "X-Bot-Secret": "adultai_discord_bot_2026",
                "Content-Type": "application/json",
            }
            try:
                async with session.post(
                    "https://adultai.com/api/discord/save-image",
                    json=payload,
                    headers=req_headers,
                    ...
                    if r.ok:
                        data = await r.json()
                        gallery_url = data.get("galleryUrl", "https://adultai.com/gallery/user")
                        claim_url = data.get("claimUrl")
                        is_new = data.get("isNewUser", False)
                        visibility = "🌍 Public" if is_public else "🔒 Private"
                        categories = data.get("categories", [])
                        saved_cat = categories[0]["name"] if categories else category_name

                        embed = discord.Embed(title="✅ Saved to AdultAI.com!", color=0xe91e8c)
                        embed.add_field(name="Visibility", value=visibility, inline=True)
                        embed.add_field(name="Category", value=f"#{saved_cat}", inline=True)
                        embed.add_field(name="\u200b", value=f"[🖼️ View in Gallery]({gallery_url})", inline=False)
                        cdn_url = data.get("cdnUrl")
                        if cdn_url:
                            embed.set_image(url=cdn_url)
                        embed.set_footer(text="Think it. See it. Feel it. — AdultAI.com")
                        if is_new and claim_url:
                            embed.description = (
                                f"🔗 **Link your Discord account** to access this image anytime:\n"
                                f"[Link account →]({claim_url})\n\n"
                                f"No account? [Sign up free →](https://adultai.com/auth/login)"
                            )
                        await interaction.followup.send(embed=embed, ephemeral=True)
                    else:
                        err = await r.text()
                        await interaction.followup.send(f"❌ Could not save image ({r.status}): {err[:100]}", ephemeral=True)
            except Exception as e:
                await interaction.followup.send(f"❌ Failed to connect to AdultAI.com: {e}", ephemeral=True)


class SaveToAdultAIButton(discord.ui.Button):
    """Opens SaveModal directly — one click, one modal, done."""
    def __init__(self, image_bytes: bytes, image_url: str = None, prompt: str = ""):
        # No custom_id — non-persistent button. Persistent buttons lose image_bytes on restart.
        super().__init__(label="💾 Save to AdultAI", style=discord.ButtonStyle.success, row=2)
        self.image_bytes = image_bytes
...
class FullImageView(discord.ui.View):
    ...
    def __init__(self, image_bytes: bytes, prompt: str):
        ...
        # Row 2: save to platform
        self.add_item(SaveToAdultAIButton(image_bytes, prompt=prompt))
                ...
                async with session.post(
                    ...
                    if resp.status == 200:
                        ...
                        if result_url:
                            ...
                            view.add_item(SaveToAdultAIButton(img_bytes, result_url, prompt=self.new_prompt.value))
                            msg = await interaction.followup.send(
                        ...
                        err = await resp.text()
                        print(f"[EDIT] img2img error {resp.status}: {err[:200]}")
            except Exception as e:
...
class AaiCog(commands.Cog, name="aai"):
    ...
    async def process_queue(self):
        while True:
            ...
            # Spawn each generation as its own task so event loop stays free
            asyncio.ensure_future(self.handle_generation(item))
            self.request_queue.task_done()

    async def handle_generation(self, item):
        interaction, prompt = item[0], item[1]
        gen_opts = item[2] if len(item) > 2 else {}
        # Guard: if interaction is already expired, skip silently
        try:
            if interaction.is_expired():
                print(f"[SKIP] Interaction already expired for prompt: {prompt[:40]}")
                return
        except Exception:
            pass
        try:
            ...
            status_msg = await interaction.followup.send(
                f"\U0001f3a8 Generating 4x images for: `{prompt}`{opts_line}  \u23f3"
            )
            ...
            async def gen_one(session, idx):
                ...
                if result:
                    ...
                    result = await loop.run_in_executor(None, apply_watermark, result)
                return (idx, result)
            ...
            if len(completed) >= 2:
                ...
                while len(imgs) < 4:
                    imgs.append(imgs[-1])
                loop = asyncio.get_event_loop()
                collage_buffer = await loop.run_in_executor(None, create_collage, imgs)
                if collage_buffer:
                    file = discord.File(collage_buffer, filename="collage.png")
                    view = ImageButtons(completed, prompt)
                    message = await interaction.followup.send(
                        f"\u2705 `{prompt}`\nClick **1\u20134** to expand  \u00b7  \U0001f504 Retry  \u00b7  \U0001f5d1 Delete",
                        file=file,
                        view=view,
                    )
                ...
                view = ImageButtons(completed, prompt)
                message = await interaction.followup.send(
                    "\u26a0\ufe0f Only 1 image generated  \u00b7  \U0001f504 Retry",
                    view=view,
                )
                view.message = message
            else:
                await interaction.followup.send(
                    "\u274c Generation failed \u2014 GPU may be busy. Try again in a moment."
                )
        except discord.errors.NotFound:
            # Interaction expired before we could respond — Discord killed it
            print(f"[EXPIRED] Interaction timed out for: {prompt[:40]}")
        except Exception as e:
            ...
            pass  # task_done called by process_queue
    ...
    @app_commands.describe(
        ...
        lora="Style LoRA to apply.",
        lora_strength="LoRA strength (0.1-1.5, default varies by LoRA).",
    )
    @app_commands.choices(
        ...
        lora=[
            app_commands.Choice(name="More Detail (enhance textures)", value="more_details"),
            app_commands.Choice(name="Detail Tweaker (fine control)", value="detail_tweaker"),
            app_commands.Choice(name="Dramatic Lighting", value="epi_noiseoffset"),
            app_commands.Choice(name="Clothing Adjuster", value="clothing_adjuster"),
            app_commands.Choice(name="Polaroid / Vintage", value="polaroid_style"),
            app_commands.Choice(name="Ghibli / Anime Art", value="ghibli_style"),
            app_commands.Choice(name="Soft / Cute", value="cute_girl_mix4"),
            app_commands.Choice(name="Anime Lineart", value="anime_lineart"),
            app_commands.Choice(name="3D Rendered / CGI", value="hipoly_3d"),
        ],
    )
    async def aai(
        self,
        ...
        if lora: gen_opts["lora_id"] = lora
        if lora_strength is not None: gen_opts["lora_strength"] = max(0.1, min(1.5, lora_strength))
